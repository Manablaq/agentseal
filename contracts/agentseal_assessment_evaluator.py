...
class AgentSealAssessmentEvaluator(gl.Contract):
    registry: Address
    ...
    def evaluate_assessment(self, snapshot_json: str, attempt_number: int, request_nonce: int) -> None:
        ...
        try:
            ...
            if type(snapshot) is not dict or set(snapshot.keys()) != {"policy", "assessment"}:
                ...
                "ASSESSMENT_SNAPSHOT_SHAPE_INVALID"
            )
            p = snapshot["policy"]
            ...
            assessment = AssessmentRecord(
                assessment_id=u256(int(a["assessment_id"])),
                subject_wallet=Address(a["subject_wallet"]),
                profile_digest=a["profile_digest"],
                endpoint=a["endpoint"],
                capability_id=a["capability_id"],
                policy_id=a["policy_id"],
                policy_version=u64(int(a["policy_version"])),
                manifest_id=a["manifest_id"],
                manifest_digest=a["manifest_digest"],
                requested_certificate_ttl=u64(int(a["requested_certificate_ttl"])),
                binding_key=a["binding_key"],
                status=a["status"],
                created_at=u64(int(a["created_at"])),
                deadline=u64(int(a["deadline"])),
                attempt_count=u64(int(a["attempt_count"])),
                max_attempt_count=u64(int(a["max_attempt_count"])),
                last_verdict=a["last_verdict"],
                case_a_id=a["case_a_id"],
                case_b_id=a["case_b_id"],
                certificate_id=u256(int(a["certificate_id"])),
            )
        ...
        result = self._semantic_evaluation_consensus(
            policy,
            assessment,
            attempt_number,
        ...
        gl.get_contract_at(self.registry).emit(on="finalized").apply_assessment_evaluation_result(
    ...
    def _semantic_evaluation_once(self, policy: PolicyRecord, assessment: AssessmentRecord, attempt_number: int, chain_id: int, contract_address: Address) -> dict:
        ...
        try:
            ...
            selection_material = self._selection_material(assessment, chain_id, contract_address)
        ...
        try:
            evaluation_id = self._evaluation_id(int(assessment.assessment_id), attempt_number, chain_id, contract_address)
            endpoint_request = self._build_endpoint_request(assessment, evaluation_id, selected_cases)
        except Exception:
            return inconclusive_selected
        try:
            endpoint_response = gl.nondet.web.request(assessment.endpoint, method='POST', headers={'content-type': 'application/json'}, body=endpoint_request)
    ...
    def _semantic_evaluation_consensus(self, policy: PolicyRecord, assessment: AssessmentRecord, attempt_number: int, chain_id: int, contract_address: Address) -> dict:

        def leader_fn() -> dict:
            return self._semantic_evaluation_once(policy, assessment, attempt_number, chain_id, contract_address)
        ...
        def validator_fn(leader_result: object) -> bool:
            ...
            if type(leader_payload) is not dict or set(leader_payload.keys()) != {'verdict', 'case_a_id', 'case_b_id'} or type(leader_payload['verdict']) is not str or (leader_payload['verdict'] not in ALLOWED_EVALUATOR_VERDICTS) or (type(leader_payload['case_a_id']) is not str) or (type(leader_payload['case_b_id']) is not str):
                return False
            validator_payload = self._semantic_evaluation_once(policy, assessment, attempt_number, chain_id, contract_address)
